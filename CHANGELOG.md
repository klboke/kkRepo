...
- V49 adds R / CRAN package, publication, snapshot, group-binding, proxy, tombstone, migration, and lease state. V50 adds the shared default UI theme setting. V51 adds a database-clock-fenced, one-time Helm proxy legacy-cache upgrade: only never-reconfigured repositories can adopt pre-migration cache rows, and later writes or configuration changes force a rebuild.
