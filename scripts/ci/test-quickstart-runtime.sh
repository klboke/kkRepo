...
grep -qx 'KKREPO_IMAGE_TAG=1.1.0' "$TMP_ROOT/default-jvm/.env"
grep -qx 'jvm|1.1.0' "$TMP_ROOT/default-jvm.log"

run_quickstart native KKREPO_RUNTIME=native
grep -qx 'KKREPO_RUNTIME=native' "$TMP_ROOT/native/.env"
grep -qx 'KKREPO_IMAGE_TAG=1.1.0-native' "$TMP_ROOT/native/.env"
grep -qx 'native|1.1.0-native' "$TMP_ROOT/native.log"
...
grep -qx 'native|1.1.0-native' "$TMP_ROOT/existing-jvm.log"
