(function () {
  ...
  const zh = {
    ...
    "Use the Browse, Search, and Upload areas with Nexus-compatible repository URLs.": "使用浏览、搜索和上传区域访问 Nexus 兼容仓库 URL。",
    "Repository Manager": "仓库管理器",
    "Hosted settings": "Hosted 设置",
    "Proxy settings": "Proxy 设置",
    "Docker connector": "Docker 连接器",
    "Optional compatibility entrypoint for repository-specific Docker traffic. Leave it disabled to use path-based Docker routing on the main service port; enable it only when clients need a dedicated /v2/ port or isolated upload/download traffic.": "用于仓库级 Docker 流量的可选兼容入口。保持禁用时使用主服务端口上的路径式 Docker 路由；仅当客户端需要独立 /v2/ 端口或隔离上传下载流量时启用。",
    ...
    "Configure the anonymous user used for unauthenticated repository reads": "配置未认证仓库读取使用的匿名用户",
    "User source": "用户来源",
    "Realm name": "Realm 名称",
    ...
    "Scopes": "Scopes",
    "Audit Log": "审计日志",
    "Search management mutations recorded in MySQL": "搜索记录在 MySQL 中的管理变更",
    ...
    "This preference is stored in MySQL and applies to the administration, browse, and sign-in UI on every replica.": "该偏好存储在 MySQL 中，并应用于所有副本上的管理、浏览和登录界面。",
    "Welcome": "欢迎",
    "Nexus-compatible repository access for internal packages": "面向内部包的 Nexus 兼容仓库访问",
    "Initial administrator setup": "初始化管理员设置",
    "Administrator": "管理员",
    ...
    "Persist configuration before checking.": "请先持久化配置再检查。",
