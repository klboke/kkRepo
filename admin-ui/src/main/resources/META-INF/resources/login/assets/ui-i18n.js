(function () {
  ...
  const zh = {
    ...
    "Sort by": "排序方式",
    "Largest stored size": "存储用量最多",
    "Most pending cleanup": "待回收用量最多",
    "Most blobs": "Blob 数量最多",
    "Logical size": "逻辑用量",
    "Stored size": "存储用量",
    "Engine / Type": "引擎 / 类型",
    ...
    "Logical asset size includes metadata and cached files. Shared content can be counted in multiple repositories.": "制品逻辑用量包含元数据和缓存文件，共享内容可能在多个仓库中分别计入。",
    ...
    "Outbound network proxy": "出站网络代理",
    "Route upstream fetches for this proxy repository through an HTTP or SOCKS proxy (e.g. a corporate Clash on the LAN). Useful when the remote registry (hub.docker.com, pypi.org, ...) is unreachable from the server.": "通过 HTTP 或 SOCKS 代理转发此 proxy 仓库的上游拉取（例如局域网内的企业 Clash）。当远端仓库（hub.docker.com、pypi.org 等）在服务器上无法直接访问时非常有用。",
    "Proxy type": "代理类型",
    ...
    "Proxy password": "代理密码",
    "Clear saved proxy password": "清除已保存的代理密码",
    "Saved proxy password unchanged": "代理密码保持不变",
    ...
    "Enable the realm and control its place in the authentication order.": "启用 Realm 并控制其在认证顺序中的位置。",
    "System managed": "系统管理",
    "Source LDAP, system managed": "来源 LDAP，由系统管理",
    "Source OIDC, system managed": "来源 OIDC，由系统管理",
    "Server connection": "服务器连接",
    "Use a complete URL, or configure the protocol, host, port, and search base separately.": "使用完整 URL，或分别配置协议、主机、端口和搜索基准。",
    "Bind account": "绑定账户",
    ...
    "Form and valid JSON changes sync automatically. Existing secrets are masked.": "表单与有效 JSON 会自动双向同步；现有密钥将脱敏显示。",
    "Synced": "已同步",
    "Waiting for valid JSON": "等待有效 JSON",
    "LDAP configuration JSON": "LDAP 配置 JSON",
    "OIDC": "OIDC",
    "Manage OIDC issuer, token validation, and claim mapping": "管理 OIDC Issuer、Token 校验和声明映射",
    "Provider settings": "提供方设置",
    "Configure the issuer and keys used to validate identity tokens.": "配置用于校验身份令牌的 Issuer 和密钥。",
    "Client configuration": "客户端配置",
    "Claim mapping": "Claim 映射",
    "Issuer": "Issuer",
    "JWKS URI": "JWKS URI",
    "Audience": "Audience",
    "Client ID": "Client ID",
    ...
    "OIDC configuration JSON": "OIDC 配置 JSON",
