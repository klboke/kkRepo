...
2. ✅ 制品安全扫描 - 已实现，默认关闭。覆盖 hosted、proxy 和迁移制品的 SBOM 与已知漏洞扫描、Docker/OCI 多平台扫描、数据库持久任务、多副本协同、policy/waiver 评估、Admin UI 和可选下载阻断（[设计说明](docs/zh/dev/security-scanning-design.md)）。
