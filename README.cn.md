...
2. ✅ 制品安全扫描 - 已实现，默认关闭。上传事务只写通用制品变更 outbox，SBOM 与已知漏洞分析完全异步；同时覆盖 Docker/OCI 多平台、数据库持久任务、多副本协同、policy/waiver 评估、Admin UI 和可选下载阻断（[设计说明](docs/zh/dev/security-scanning-design.md)）。
