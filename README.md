...
| Terraform Provider / Module Registry | hosted / proxy / group | Nexus-compatible PUT and UI/API archive upload; `terraform init` resolves hosted and proxied modules/providers through groups | Module/provider coordinates, versions, platforms, Browse/Search, and Usage supported | Nexus Terraform hosted data is migrated; proxy configuration migrates, but proxy cache data import is rejected until route/checksum/signature snapshot evidence is available |
...
Repository data migration scans hosted repositories by default. If you need to migrate proxy repositories from a source Sonatype Nexus Repository deployment as historical backup data or upstream cache data, explicitly specify repository names in `Optional proxy repositories` on the migration page. Cargo / Rust, Dart / Pub, and native Nexus Composer proxy-cache migration are supported when explicitly selected and accepted by the source profile. Terraform proxy configuration migrates, but Terraform proxy cache data import currently fails fast before a repository-data job is created.
...
5. ✅ Terraform Provider / Module Registry - Hosted, proxy, group, provider GPG signing, Nexus-compatible paths, UI/API upload, search, real Terraform CLI E2E, and Nexus hosted-data/configuration migration implemented ([Chinese design notes](docs/zh/dev/terraform-repository-design.md))
