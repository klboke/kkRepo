...
-- soft-delete UPDATE while a new document owner exists.
ALTER TABLE asset_blob
  ADD COLUMN external_reference_count BIGINT NOT NULL DEFAULT 0,
  ADD CONSTRAINT ck_asset_blob_external_reference_nonnegative
    CHECK (external_reference_count >= 0) NOT VALID,
  ADD CONSTRAINT ck_asset_blob_external_reference_live
    CHECK (external_reference_count = 0 OR deleted_at IS NULL) NOT VALID;
...
CREATE INDEX idx_security_scan_task_projection
  ON security_scan_task(asset_id, profile_id, content_generation, id);
...
CREATE INDEX idx_security_scan_task_repository_profile
  ON security_scan_task(repository_id, profile_id, id);
...
CREATE INDEX idx_security_scan_finding_run_severity
  ON security_scan_finding(scan_run_id, severity, id);
