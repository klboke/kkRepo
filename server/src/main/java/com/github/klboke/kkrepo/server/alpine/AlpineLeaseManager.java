...
/** Renewable database lease with fencing tokens for cross-replica Alpine publication. */
@Component
final class AlpineLeaseManager {
  ...
  @Autowired
  AlpineLeaseManager(AlpineRegistryDao registry) {
    this(registry, DEFAULT_TTL, DEFAULT_WAIT);
  }

  AlpineLeaseManager(AlpineRegistryDao registry, Duration ttl, Duration wait) {
  ...
  Lease acquire(String key) {
    ...
    do {
      ...
      if (acquired.isPresent()) {
        AlpineRegistryDao.Lease row = acquired.orElseThrow();
  ...
  private static MavenExceptions.WritePolicyDenied busy(String key) {
    return new MavenExceptions.WritePolicyDenied(
        "Another replica owns this Alpine namespace; retry the request: " + key);
  }
  ...
  static final class Lease implements AutoCloseable {
    ...
    private Lease(
      ...
      this.ttl = ttl;
      this.renewal = Thread.ofVirtual().name("alpine-publish-lease-renewal").start(this::renew);
    }
