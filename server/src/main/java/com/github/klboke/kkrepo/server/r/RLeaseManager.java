...
/** Renewable database lease with fencing tokens for cross-replica R publication. */
@Component
final class RLeaseManager {
  ...
  Lease acquire(String key) {
    ...
    do {
      ...
      if (acquired.isPresent()) {
        RRegistryDao.Lease row = acquired.orElseThrow();
  ...
  static final class Lease implements AutoCloseable {
    ...
    private Lease(
      ...
      this.ttl = ttl;
      this.renewal = Thread.ofVirtual().name("r-publish-lease-renewal").start(this::renew);
    }
