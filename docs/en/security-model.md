...
Completion pages use timestamp/ID indexes added by migration V55. Tasks read completed and unfinished ranges separately, with at most the remaining page size fetched from each range; the server reads both in one read-only transaction. This avoids a history-wide null-rank sort while keeping unfinished tasks last. MySQL builds the indexes online; PostgreSQL builds them concurrently. The migrations support retry after interruption.
