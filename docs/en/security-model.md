...
Query-bearing NuGet group links carry an authenticated routing proof bound to the entry group, source member, package path and exact query. Every replica verifies it with the existing deployment credential secret; ordinary repository authorization still applies, and membership is checked against the current group configuration. Client queries cannot override resource-owned parameters. Failed signed download requests are excluded from the canonical negative cache.
