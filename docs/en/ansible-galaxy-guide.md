...
Each `(namespace, name, version)` is immutable. The multipart route derives the coordinate from the required canonical archive filename and rejects an already-published or actively reserved version before staging. Task creation atomically reserves the coordinate in the database, so a concurrent in-flight duplicate is rejected instead of receiving a second `202`; any staging reference created during a race is unlinked. Multipart task creation, direct PUT, and worker promotion use the same fenced coordinate lease and recheck both the immutable version and active reservation inside it. A task retains its publication provenance so crash recovery is idempotent. Re-publishing the same version fails even when the repository write policy otherwise allows updates. Build a new collection version instead.
