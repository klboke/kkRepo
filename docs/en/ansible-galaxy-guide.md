...
Each `(namespace, name, version)` is immutable. The multipart route derives the coordinate from the required canonical archive filename and rejects an already-published version before staging. Task creation atomically reserves the active coordinate in the database, so a concurrent in-flight duplicate is rejected instead of receiving a second `202`; any staging reference created by the losing request is unlinked. A task retains its publication provenance so crash recovery is idempotent. Re-publishing the same version fails even when the repository write policy otherwise allows updates. Build a new collection version instead.
