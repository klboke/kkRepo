...
Database aggregates are read as arbitrary-precision integers, including sums above
Java’s `long` range. Counts and byte totals are transferred as decimal strings
and calculated with `BigInt` in the UI. Values above 8 PiB, adjacent large counts, and filtered sums
