...
kkRepo is a self-hosted artifact repository that follows Nexus-style client access patterns. It supports Maven, npm, PyPI, Go, Helm, Cargo/Rust, Dart/Pub, Composer/PHP, Terraform, Docker/OCI, and other artifact formats. For Maven usage, it keeps the familiar `/repository/<repo>/...` URL layout, which lowers the client-side configuration cost when migrating from or replacing Nexus.
