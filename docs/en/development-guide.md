...
This suite publishes/uploads and then downloads/resolves through Maven, npm, PyPI, Go, Helm, Cargo/Rust, Dart/Pub, Composer/PHP, Terraform 0.13/current, SwiftPM/Xcode, Ansible Galaxy 2.9/current, Conda, APT/Debian, NuGet, RubyGems, Yum, and Docker/OCI clients. Composer additionally covers a hosted-to-proxy transitive dependency, rejected Basic credentials, and lock replay after clearing the client cache and detaching the upstream; Terraform covers hosted and registry.terraform.io proxy modules/providers through a group, including provider checksum/signature metadata; Swift 5.7 covers registry/proxy resolve/build, while 5.10/6.x additionally cover HTTPS login, immutable publication, hosted/group flows, SCM replacement, and cross-replica reads; Ansible covers Galaxy v3 discovery, durable publish tasks, immutable versions, range/transitive dependencies, hosted/group download, public Galaxy proxying, Basic/GenericToken/Nexus bearer authentication, and cross-replica reads; Conda covers hosted upload, group search/create/list, upstream proxy search/create/list, repodata variants, and cleanup; APT covers three client images, authenticated signed install/upgrade/delete, key rotation, passthrough/re-sign proxying, and cross-replica reads. Dedicated Swift lanes cover macOS Xcode and Windows proxy resolution. Go publishes a canonical internal module ZIP to hosted, resolves it through group, and verifies public proxy fallback through the same group. Client diagnostics are written under `artifacts/client-e2e/`, with credential-bearing metadata redacted before artifact upload.
...
- [Go Repository Guide](repository-guides/go.md) and [Nexus Performance Baseline](dev/go-hosted-performance-baseline.md)
