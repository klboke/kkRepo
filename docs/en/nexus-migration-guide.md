...
Terraform keeps the Nexus `/repository/<repo>/v1/modules/...` and `/v1/providers/...` service bases configured in Terraform CLI `host.services`. Metadata migration recognizes native `terraform-hosted`, `terraform-proxy`, and `terraform-group` recipes. Repository-data migration rebuilds hosted module/provider state, provider platforms, SHA256SUMS, and kkrepo signing metadata. Proxy/group configuration migrates, but Terraform proxy cache data import is rejected before job creation because the current source profile cannot prove remote route, validator, checksum, and signature snapshot semantics.
