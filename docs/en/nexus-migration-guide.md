...
Swift Package Registry keeps the Nexus `/repository/<repo>/` registry base. Metadata migration recognizes `swift-hosted`, `swift-proxy`, and `swift-group` and preserves proxy TTLs and ordered members. A recoverable source proxy secret is encrypted in the target. A masked or missing source secret instead produces `NEEDS_MANUAL_ACTION`; the target proxy stays offline with no placeholder credential until an administrator supplies one. Hosted data migration restores immutable source archives, default/versioned manifests, optional CMS signatures, original release metadata, and repository URL mappings only for Nexus 3.92.x-3.94.x sources whose datastore asset fingerprint proves the expected Swift content shape. Out-of-range versions, unknown profiles, and shape drift fail closed; proxy cache data is not replayed as hosted publication.
...
Each lane verifies definition migration, signed archive/default and versioned manifests, original metadata, checksum, repository URL mapping, cross-replica reads, restart/resume, and exact row-count idempotency. It also verifies the fail-closed proxy-secret path and explicit target-side credential completion. The 3.92.x-3.94.x version/shape gate is covered by source-profile contracts; the live matrix does not imply support for versions outside that range or for an unrecognized datastore shape.
