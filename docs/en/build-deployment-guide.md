...
`KKREPO_RUNTIME=jvm` selects `ghcr.io/klboke/kkrepo:1.1.0`, while `KKREPO_RUNTIME=native` selects `ghcr.io/klboke/kkrepo:1.1.0-native`. Both are published for Linux `amd64` and `arm64`. An explicit `KKREPO_IMAGE_TAG` overrides the runtime's default tag.
...
- The JVM runtime from `ghcr.io/klboke/kkrepo:1.1.0`
- MySQL 8.0
