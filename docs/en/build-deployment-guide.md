...
`KKREPO_RUNTIME=jvm` selects `ghcr.io/klboke/kkrepo:1.0.2`, while `KKREPO_RUNTIME=native` selects `ghcr.io/klboke/kkrepo:1.0.2-native`. Both are published for Linux `amd64` and `arm64`. An explicit `KKREPO_IMAGE_TAG` overrides the runtime's default tag.
...
- The JVM runtime from `ghcr.io/klboke/kkrepo:1.0.2`
- MySQL 8.0
...
The `.env` encryption secrets generated by the script are only for local trials. For externally reachable deployments, replace `KKREPO_CREDENTIAL_SECRET` and `KKREPO_API_KEY_PAYLOAD_SECRET`, and use an independent MySQL/PostgreSQL instance plus OSS/S3 blob storage.
...
The default configuration connects to MySQL at `127.0.0.1:13306/kkrepo`. If your local database uses a different port or account, override it with Spring Boot environment variables and declare the type:
...
For PostgreSQL, use `KKREPO_DATABASE_TYPE=postgresql` and a `jdbc:postgresql://...` URL.
...
- Database: at least 2C4G for production. Use an independent MySQL/PostgreSQL instance and scale CPU, memory, disk, and IOPS based on repository count, package count, index size, and migration workload.
...
- Use an independent relational database and an independent OSS/S3 bucket in production. Do not use File blob store as long-term production storage.
- In multi-replica deployment, sessions, authentication tickets, catalog watermarks, locks, and migration progress are coordinated through the database. Losing node-local cache does not affect correctness.
...
When retaining the same database and OSS/S3 blob store, back up both before upgrading and confirm the new backend-specific Flyway migrations are expected.
