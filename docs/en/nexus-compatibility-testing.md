...
The current module includes compatibility test classes for Maven, npm, PyPI, Go, Helm, Cargo/Rust, Dart/Pub, Composer/PHP, Terraform, Swift Package Registry, Ansible Galaxy, Conda, APT/Debian, Alpine/APK, R/CRAN, Docker/OCI, NuGet, RubyGems, Yum, Raw, component upload, security management APIs, and related areas.
...
The unified Nexus compatibility matrix uses the datastore-era Nexus PostgreSQL reference for newer formats. Run `scripts/ci/run-live-compat.sh nexus` against the disposable compose environment to compare kkrepo with Nexus across Maven, npm, PyPI, Cargo/Rust, Dart/Pub, Composer/PHP, Terraform, Raw, selected NuGet/RubyGems/Yum behavior, Go hosted/proxy/group endpoints, Helm hosted round trips and group index aggregation, component upload specs, and selected security/admin contracts. Composer and Terraform reference endpoints are required when enabled; a missing capability fails instead of skipping. This suite enables disposable write checks by default. The live `nexus` workflow selects Nexus 3.94 so Go hosted and the other newest recipes are exercised. In GitHub Actions, use the `Live Compatibility / Nexus compatibility` job through the `run-live-compat` label or the scheduled workflow.

Outside Dart / Pub, some component upload specs and security/admin contracts still compare Nexus legacy UI/internal API routes. kkrepo production and distribution configuration keeps `kkrepo.nexus.legacy-ui.enabled=false` by default to disable the Nexus Rapture, ExtDirect, Wonderland, internal UI REST, iframe upload, and related legacy web UI compatibility surface as one coarse-grained switch. The disposable compatibility compose environment sets `KKREPO_NEXUS_LEGACY_UI_ENABLED=true` for the candidate service by default only for those Nexus legacy-route compatibility checks. This is a startup-time runtime flag: when reproducing these live tests manually, set the environment variable before starting or restarting the candidate kkrepo service; setting it only before `mvn -pl compat-test ... test` does not affect an already-running service. Normal deployments should keep the default disabled, and the kkrepo Browse UI uses the native `/internal/security/permissions` endpoint instead of the legacy internal UI permissions route.
...
Go compatibility uses Nexus 3.93+ hosted/proxy/group recipes, with Nexus 3.94 in the live workflow. `GoProxyBlackBoxCompatibilityTest` compares repository-root module ZIP publication, list/info/mod/zip/latest and HEAD behavior, generated metadata/media types, hosted-first group reads, and ordered proxy fallback. The real-client suite uploads an internal module and resolves both that module and a public proxied module through one group.
...
Conda compatibility targets Nexus 3.92.x-3.94.x. `CondaRepositoryBlackBoxCompatibilityTest` always runs self-contained archive/path/metadata fixtures; its opt-in live branch creates isolated hosted/proxy/group repositories on Nexus and kkrepo and compares root/nested channels, `.tar.bz2`/`.conda` publication, JSON/BZ2/ZSTD/current repodata, channeldata, group priority, package bytes/checksums, and conditional requests. Enable it with the `CONDA_COMPAT_*` variables documented in [compat-test README](../../compat-test/README.md). Real-client coverage uses Miniforge Conda to exercise hosted upload, group and proxy `search/create/list`, package bytes, metadata variants, and cleanup.

APT compatibility targets Nexus 3.92.x-3.94.x, with the dedicated live suite pinned to Nexus 3.94.x. `AptRepositoryBlackBoxCompatibilityTest` creates isolated signed hosted repositories and compares repository-root/Components uploads, canonical pool paths, Packages variants, Release/InRelease signatures, checksums, validators, Range, Browse, and Search. The real-client matrix runs Debian 12, Ubuntu 24.04, and Debian testing containers through authenticated hosted, passthrough/offline proxy cache, re-signing, key rotation, delete/upgrade, and dual-replica flows. Enable it with `scripts/ci/run-live-compat.sh apt`; exact variables are documented in [compat-test README](../../compat-test/README.md).

Alpine/APK compatibility targets Nexus 3.94.x. `AlpineRepositoryBlackBoxCompatibilityTest`
creates isolated signed hosted repositories and compares raw APK v2 upload, signed
...
R/CRAN compatibility targets Nexus 3.94.x. `RRepositoryBlackBoxCompatibilityTest` creates
isolated hosted repositories, uploads the same deterministic source package, normalizes and
compares `PACKAGES.gz` DCF records, and checks package GET/HEAD/Range/validators, Browse, and
Search. The real-client matrix runs R 4.5.3 and 4.6.1 through hosted/group
`available.packages()`, dependency installation, update, proxy `PACKAGES.gz`/`PACKAGES.rds`,
version-order differential checks, and optional second-replica reads.

Evidence stays at the appropriate layer: the compatibility black box covers active/revoked/expired `GenericToken` behavior, while server/persistence tests cover moving-tag immutability, 1,200-tag pagination bounds, cleanup, and 429/5xx propagation. The scheduled Swift resilience lane uses two replicas and RustFS through the AWS S3-compatible adapter to exercise a multi-megabyte package, shared 429/5xx waterlines with stale fallback, lease takeover, restart, and destructive relational-database/object backup-restore. Alibaba OSS Native has adapter-contract coverage but no live endpoint E2E claim.
...
It runs publish/upload plus download/resolve flows for Maven, npm, PyPI, Go, Helm, Cargo/Rust, Dart/Pub, Composer/PHP, Terraform 0.13/current, SwiftPM/Xcode, Ansible Galaxy 2.9/current, Conda, APT/Debian, Alpine/APK, R 4.5/4.6, NuGet, RubyGems, Yum, and Docker/OCI. Go uploads a canonical internal module to hosted, resolves it through group, and then resolves a public module through the same group's proxy fallback. Helm uploads a private chart to hosted and uses one group endpoint to pull both that chart and a public chart through proxy fallback. Composer covers hosted archives, group resolution, Packagist proxying, a transitive dependency, Basic authentication, and lock replay from the server cache. Terraform covers hosted module/provider upload, registry.terraform.io proxying, group resolution, URL-token authentication, checksums, and signatures. Swift 5.7 covers registry/proxy resolve/build; 5.10/6.x additionally cover hosted publication, Basic/GenericToken login, group resolution/build, GitHub SCM replacement, checksum replay, and cross-replica reads. Ansible builds dependent collections, publishes through hosted, rejects duplicate versions, installs/downloads through group, verifies GenericToken/Basic/anonymous behavior, exercises a public Galaxy proxy, and can read through a second replica. Conda uploads a generated installable package, resolves it through group, resolves a controlled upstream package through proxy, verifies JSON/BZ2/ZSTD repodata and downloaded bytes, and executes the shared cleanup gate. APT validates signed authenticated installs, upgrades, deletes, key rotation, proxy modes, and cross-replica reads with three client images. Alpine validates signed metadata and exact package bytes with apk-tools 2.x/3.x across hosted, proxy, group, deletion, and a second replica. R validates hosted/group metadata, dependency install/update, proxy metadata passthrough, package bytes, official version ordering, and optional second-replica reads. macOS and Windows have dedicated Xcode and proxy-only lanes. Docker image push/pull is always covered, and ORAS pushes/pulls a generic OCI artifact when the `oras` client is available.
...
Client command logs, downloaded metadata, selected inspect output, and other diagnostics are written under `artifacts/client-e2e/`. Terraform metadata may contain URL tokens and is redacted before upload. The suite depends on the real tools listed in [compat-test README](../../compat-test/README.md), so a local workstation may need extra SDKs or package managers before it can run the full matrix.

## Migration E2E

The `Migration E2E` workflow imports configuration, security metadata, and repository data from supported Nexus generations, including the historical 3.29.x embedded/OrientDB reference and datastore-era H2/PostgreSQL references. The live matrix selects representative Nexus persistence eras and internal database/content-model shapes; it does not enumerate every patch release. An existing lane represents later releases while the migration-visible database schema and source fingerprint remain unchanged, and a new version lane is added only when the persistence engine, schema/content shape, or migration behavior changes. Every real source is still fingerprinted during preflight and unknown drift fails closed. Swift hosted data is `FULL` only for verified Nexus 3.92.x-3.94.x datastore shapes; out-of-range versions or shape drift remain manual. The live Swift matrix uses Nexus 3.94 and covers H2 to MySQL plus PostgreSQL source to MySQL/PostgreSQL targets, restart/resume, exact-row-count idempotency, and fail-closed masked/missing proxy secrets. It also proves the native Nexus 3.94 persistence boundary: uploaded optional signatures, original metadata, and repository URLs are absent from the source export and are not fabricated in the target; separate writer contracts preserve them when an export actually contains them. Ansible migration contracts enforce the Nexus 3.93.x-3.94.x version/shape gate, protocol-aware archive restoration, immutable idempotency, opt-in proxy cache, and fail-closed proxy secrets. Complete collection manifest/files JSON stays in blob storage rather than relational JSON columns. Such a proxy stays offline without a placeholder credential until the target secret is supplied explicitly. Conda lanes cover Nexus 3.92 PostgreSQL and Nexus 3.94 H2/PostgreSQL sources, both kkrepo target databases, native hosted/proxy/group definitions, an installable hosted package, generated-metadata filtering/rebuild, real client installation, exact checksums and protocol row counts, restart/resume idempotency, and cross-replica reads where available. APT lanes cover Nexus 3.92 PostgreSQL and Nexus 3.94 H2/PostgreSQL shapes, filter generated metadata, restore checksum-verified `.deb` content, keep the target offline until an explicit signing-key import, rebuild signed metadata, run a real `apt` install, verify protocol row counts, and read through a second replica where available. Alpine lanes use the exact Nexus 3.94 H2/PostgreSQL shape, migrate native hosted/proxy/group definitions and checksum-verified APK v2 content to both target databases, keep local-signing repositories offline until explicit PKCS#8 key import, rebuild signed indexes, run a real dependency install, and verify generated-index filtering, row counts, idempotency, and cross-replica reads. Add the `run-migration-e2e` label to a PR when a change affects source detection, migration adapters, repository-data import, blob/checksum validation, permissions, or post-migration protocol behavior.

Go hosted migration is enabled only on the Nexus 3.94 lanes because Sonatype introduced Go hosted
and group repositories in 3.93. Those lanes publish a native module ZIP before preflight, require a
`FULL` datastore-backed plan, migrate the hosted definition and module assets, compare the ZIP
checksum, and run `go mod download` through both kkrepo replicas.
...
- Confirm that real Maven, npm, PyPI, Go, Helm, Cargo/Rust, Dart/Pub, Composer/PHP, Docker/OCI, Terraform, SwiftPM/Xcode, Ansible Galaxy, Conda, Alpine/APK, and similar client requests are recognized correctly by kkrepo.
...
Cargo / Rust, Dart / Pub, Terraform, Swift, Ansible Galaxy, Conda, APT/Debian, and Alpine/APK are not included in the historical production-scale validation numbers above. Validate Cargo with the Nexus 3.77.x+ compatibility suite and real Cargo clients, validate Pub with the Nexus 3.92.0+ compatibility suite plus real `dart pub` / `flutter pub` clients, validate Terraform with the Nexus 3.90.0+ reference plus Terraform 0.13/current `terraform init`, validate Swift with the Nexus 3.94.x reference plus the cross-platform SwiftPM/Xcode matrix, validate Ansible with the Nexus 3.94.x `ansible` suite plus Ansible 2.9/current `ansible-galaxy`, validate Conda with the Nexus 3.92.x-3.94.x black box plus real group/proxy `conda search/create/list`, validate APT with the Nexus 3.94.x black box plus Debian/Ubuntu/current `apt update/download/install/upgrade`, and validate Alpine with the Nexus 3.94.x black box plus apk-tools 2.x/3.x `update/search/policy/fetch/add/upgrade` before production cutover.
