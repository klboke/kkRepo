...
The validation classes listed below are black-box protocol checks. The `client-e2e` suite adds real package-client coverage across Maven, npm, PyPI, Go resolve, Helm, Cargo/Rust, Dart/Pub, Composer/PHP, Terraform 0.13/current, SwiftPM/Xcode, Ansible Galaxy 2.9/current, Conda, APT/Debian, NuGet, RubyGems, Yum, and Docker/OCI; see [compat-test README](../../compat-test/README.md) for the runner requirements and `artifacts/client-e2e/` diagnostics.
