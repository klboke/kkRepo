...
- APT supports binary `.deb` repositories through hosted and proxy recipes; Nexus-compatible APT group is intentionally not exposed. Hosted source packages, flat hosted repositories, PDiff, generated Contents/Translation indexes, and `.udeb` indexes are outside the current support boundary. Proxy passthrough can serve those upstream paths unchanged, while local re-sign metadata only represents checksum-verified cached binary packages.
