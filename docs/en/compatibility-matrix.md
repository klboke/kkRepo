...
The validation classes listed below are black-box protocol checks. The `client-e2e` suite adds real package-client coverage across Maven, npm, PyPI, Go resolve, Helm, Cargo/Rust, Dart/Pub, Composer/PHP, Terraform 0.13/current, SwiftPM/Xcode, Ansible Galaxy 2.9/current, Conda, APT/Debian, Conan 2, Alpine/APK, NuGet, RubyGems, Yum, and Docker/OCI. Hugging Face uses its opt-in Nexus matrix plus the recorded Hub/Transformers/Diffusers client run. See [compat-test README](../../compat-test/README.md) for runner requirements and `artifacts/client-e2e/` diagnostics.
