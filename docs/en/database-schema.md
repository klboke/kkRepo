...
| Coordination | `cache_version`, rebuild markers, `maintenance_cursor`, proxy, Docker, Terraform, Swift, Ansible, Conda, APT, Alpine, and R lease/publish state | Cache invalidation, claim/retry work, and shared runtime state |
| Migration | `migration_job`, checkpoints, validation, repository/asset migration tables | Resumable Nexus migration and reporting |
| Protocol side tables | Docker/OCI, Pub, Terraform, Swift, Ansible Galaxy, Conda, APT, Alpine, R, and other protocol-specific relations | State that cannot be represented only by common asset attributes; Terraform adds signing keys, provider revisions/platforms, group source bindings, and publish leases in V30; Swift V31 adds releases/manifests/SCM URLs, GitHub source pins, group bindings, leases, tombstones, negative cache, and repository revisions; Ansible V35 adds collection versions/signatures, durable import tasks, proxy version state, group source bindings, and fenced registry leases; Conda V41 adds package/channel projections, record fingerprints, metadata projection indexes, tombstones, group source bindings, and fenced coordinate leases; APT V42 adds package/tombstone projections, suite revisions, immutable published snapshots, encrypted signing-key revisions, proxy distribution state, and fenced publish leases; Alpine V46 adds package/relation/tombstone projections, namespace revisions, immutable signed index snapshots, encrypted signing-key revisions, proxy index state, group source bindings, and fenced publish leases; R V49 adds source-package/relation/tombstone projections, suite revisions, immutable `PACKAGES.gz` snapshots, proxy distribution identity, snapshot-scoped group bindings/staging, and fenced leases |
