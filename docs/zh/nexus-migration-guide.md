...
对于 source profile 已确认支持 Cargo content model 的 Nexus 3.77.x+ datastore 时代 H2/PostgreSQL 源端，Cargo / Rust hosted 仓库数据通过此流程迁移。对于 source profile 已确认 Pub content model 的 Nexus 3.92.0+ datastore 源端，Dart / Pub hosted 仓库数据也通过此流程迁移；Pub proxy cache 仅在显式选择且计划为 `FULL` 时迁移。Nexus 原生 Composer 与 Terraform proxy cache 同样要求管理员显式选择，并由 source profile 证明相应 datastore content model；缺少产品能力、未知 schema、community plugin 或不支持的 repository type 都会 fail closed。旧 OrientDB 源端不会启用 datastore-only 内容导出。
