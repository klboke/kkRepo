...
Swift Package Registry 保持 Nexus `/repository/<repo>/` registry base。元数据迁移识别 `swift-hosted`、`swift-proxy`、`swift-group`，保留 proxy TTL 和有序成员。可恢复的源 proxy secret 会加密写入目标；被遮蔽或缺失的 secret 会生成 `NEEDS_MANUAL_ACTION`，目标 proxy 保持 offline 且不写入占位 credential，直到管理员显式补齐。Hosted 数据仅对 Nexus 3.92.x-3.94.x 且 datastore asset fingerprint 能证明预期 Swift content shape 的源，恢复不可变 source archive、默认/版本化 manifest、可选 CMS 签名、原始 release metadata 和 repository URL mapping。版本超出范围、未知 profile 或 shape 漂移均 fail closed；proxy cache 不会作为 hosted publication 重放。
...
- 确认 kkrepo 已经部署并连接生产 MySQL 或 PostgreSQL。
...
对于 source profile 已确认支持 Cargo content model 的 Nexus 3.77.x+ datastore 时代 H2/PostgreSQL 源端，Cargo / Rust hosted 仓库数据通过此流程迁移。对于 source profile 已确认 Pub content model 的 Nexus 3.92.0+ datastore 源端，Dart / Pub hosted 仓库数据也通过此流程迁移；Pub proxy cache 仅在显式选择且计划为 `FULL` 时迁移。Nexus 原生 Composer 和 Terraform proxy cache 同样要求管理员显式选择，并由 source profile 证明原生 content model。Swift hosted 仅在 Nexus 3.92.x-3.94.x 且 Swift datastore fingerprint 已验证时可规划为 `FULL`，并通过协议感知 writer 恢复 release；版本超出范围、fingerprint 不完整或 shape 漂移均生成 manual action。Terraform 只导入协议可识别的 archive asset；module download discovery 可直接使用已恢复 path，Provider route/checksum/signature 状态由目标上游重建。缺少产品能力、未知 schema、community plugin、不支持的 repository type 或低于 `FULL` 的计划都会 fail closed。旧 OrientDB 源端不会启用 datastore-only 内容导出。
...
当前 Swift 迁移 workflow 使用 Nexus 3.94 fixture，并运行以下源/目标数据库 lane：

| Nexus 源元数据 | kkrepo 目标 |
| --- | --- |
| Datastore H2 | MySQL |
| Datastore PostgreSQL | MySQL |
| Datastore PostgreSQL | PostgreSQL |

每个 lane 都校验 repository definition、已签名 archive、默认/版本化 manifest、原始 metadata、checksum、repository URL mapping、跨副本读取、restart/resume 和精确行数幂等，并验证 proxy secret fail-closed 与目标端显式补齐 credential。3.92.x-3.94.x 的版本/shape gate 由 source-profile contract 覆盖；该 live 矩阵不代表支持范围外版本或未识别 datastore shape。
...
迁移任务、仓库扫描游标、asset 状态和失败信息都存储在共享数据库中。迁移过程中如果 kkrepo 重启、网络中断、源 Nexus 短暂不可用或页面关闭，可以继续执行：
...
确认无问题后，把原 Nexus 域名通过 DNS 或反向代理指向 kkrepo。由于 kkrepo 兼容 Nexus 的 `/repository/<repo>/...` URL 布局、客户端协议和权限认证模型，对于已迁移的仓库格式，客户端不需要修改 Maven settings、npm registry、PyPI index-url、Go GOPROXY、Helm repo、Cargo sparse registry URL、Pub hosted URL、Composer repository URL、Terraform `host.services` URL、Swift registry base 等配置。
