...
协议客户端仍可使用各自原生认证机制。当前协议 token domain 包含 `NpmToken`、`CargoToken`、`NuGetApiKey` 和 `RubyGemsApiKey`；对应客户端协议使用 token 或 API key 时按各自协议处理，其中 Cargo 和 RubyGems 客户端通过 `Authorization` header 发送 registry/API key token。`GenericToken` 用于不绑定特定协议 token 格式、但需要仓库 API-key 认证的 CI、脚本和 HTTP 客户端。
