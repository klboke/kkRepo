...
协议客户端仍可使用各自原生认证机制。当前协议 token domain 包含 `NpmToken`、`CargoToken` 和 `NuGetApiKey`；对应客户端协议使用 token 或 API key 时按各自协议处理，其中 Cargo 客户端通过 `Authorization` header 发送 registry token。
