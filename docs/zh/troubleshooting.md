...
- Terraform 迁移被阻断时，确认源仓库使用受支持 Nexus 版本的原生 `terraform-hosted`/`terraform-proxy` recipe。Proxy cache 必须显式选择；未知 source-profile fingerprint 会 fail closed，不会导入未经验证的 route 或 signing metadata。
