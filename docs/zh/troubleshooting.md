...
- Terraform 迁移被阻断时，确认源仓库使用受支持 Nexus 版本的原生 `terraform-hosted` recipe。Terraform proxy/group 配置可迁移，但 proxy cache 数据 backup 会在创建 job 前被明确拒绝，避免导入未经验证的 route、validator、checksum 和 signing snapshot。
