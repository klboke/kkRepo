...
- 使用方需要逐个修改 CI、构建脚本、Maven settings、npm registry、PyPI index-url、Go proxy、Helm repo、Cargo registry 配置、Pub hosted URL、Composer repository URL、Terraform `host.services` 等配置。
...
- 原制品仓库域名指向 kkRepo。
- 客户端配置不需要修改。
- CI 继续使用原来的 Maven settings、npm registry、PyPI index-url、Go proxy、Helm repo、Cargo registry 配置、Pub hosted URL、Composer repository URL 和 Terraform `host.services` URL。
- 使用方没有感知到迁移过程。
- 迁移后没有收到业务侧失败反馈。
