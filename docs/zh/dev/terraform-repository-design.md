...
5. 管理、搜索和迁移
   ...
   - Nexus metadata migration 识别 Terraform repository definition；repository data migration 对 hosted 支持 dry-run、resume、checksum、报告和 source profile gate。Proxy/group 配置可迁移；显式选择且计划为 `FULL` 的原生 Nexus Terraform proxy 通过独立 cache writer 恢复 module/provider archive，保留 Nexus 公开 path，并由目标端从已配置上游重建 route/validator/checksum/signature snapshot。
