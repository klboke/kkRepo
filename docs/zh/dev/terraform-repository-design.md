...
- Module 下载 metadata 中可直接识别为 archive 的 `X-Terraform-Get`，以及 Provider 下载 metadata 中的 `download_url`、`shasums_url`、`shasums_signature_url`，必须指回当前 hosted/proxy/group 路径；HTTP vanity、VCS 和带 `//subdir` 的 go-getter source 保持官方语义，绝对地址原样透传，相对地址先按官方协议基于上游完整 download URL 解析。反向代理下使用可信外部 base URL。
- URL token 可能出现在 `/v1/modules/{token}/...` 或 `/v1/providers/{token}/...` 服务基址中。缓存中不得保存调用者 token；只缓存 token-free 规范化 metadata，请求返回时再渲染 URL。
- 私有仓库的 module/provider download metadata 若通过普通 Basic 或 API key 认证，必须把已验证且可重放的 credential 编码为 URL-token segment 后再渲染后续 archive/checksum URL；请求本身通过匿名身份读取时保持裸 URL。session/OIDC 等不可安全重放的认证即使 metadata path 可匿名读取也必须返回认证挑战，不能据此推断 content selector 同样允许后续 archive/checksum path，更不能生成失效 URL。
...
2. Terraform proxy
   ...
   - Module 的 `X-Terraform-Get` 本地映射持久化为内部 asset；metadata TTL 内直接复用。若映射缺失但 Nexus 公开 path 下已有迁移/缓存 archive，download discovery 直接选择该本地 archive，archive route 缺失时也从已迁移 blob 服务。
   - Provider archive 必须同时通过响应中的 `shasum` 和 SHA256SUMS 对应行校验；签名和 signing key 原样保留，不由 proxy 冒充上游重新签名。
   - Provider download、SHA256SUMS 和 signature 构成同一验证快照；带 expected SHA-256 的 route 使用 metadata TTL 固定已缓存 blob，同时仍使用 content timeout 回源，不能让更短的 content TTL 单独替换快照成员。
...
5. 管理、搜索和迁移
   ...
   - Nexus metadata migration 识别 Terraform repository definition；repository data migration 对 hosted 支持 dry-run、resume、checksum、报告和 source profile gate。Proxy/group 配置可迁移；显式选择且计划为 `FULL` 的原生 Nexus Terraform proxy 通过独立 cache writer 恢复 module/provider archive 并保留 Nexus 公开 path。Module download 可直接发现已恢复 archive；Provider route/validator/checksum/signature snapshot 由目标端从已配置上游重建并固定到 metadata 有效期。
...
Proxy 将 direct HTTP module 的本地 archive 映射持久化到共享 asset/blob 真相并按 metadata TTL 复用；显式迁移的 Nexus proxy archive 即使没有 kkrepo 内部 remote route，也可由 coordinate/version path 直接发现和服务。不得依赖单 JVM map 保存 `X-Terraform-Get`。
...
3. 可缓存 archive 返回当前 proxy/group archive path；HTTP vanity、VCS 和其它 go-getter source 保留 Terraform 客户端负责解释的 query 与子目录语义，其中绝对地址原样透传，相对地址基于上游完整 download URL 解析后再返回。
...
- Token segment 必须在所有 access/error 日志和 CI artifact 中脱敏；文本响应 artifact 要在任何可能失败的后续断言前完成脱敏，二进制 artifact 不得经过文本改写。脱敏识别必须先于仓库存在性和 format 判断，删除中的仓库、未知仓库或错误 format 也不能泄露 Terraform-shaped URL 中的 token。审计只记录 token id、principal、repository、operation 和结果，不记录 token material。
...
- Component、asset、Provider platform、signing state、source binding、module download mapping、proxy validator、negative cache、revision、迁移 checkpoint 和 blob reference 以共享关系数据库为真相。
...
- Proxy cache：管理员必须在 `Optional proxy repositories` 中显式选择；只有 source profile 证明原生 Terraform content model 且计划为 `FULL` 时才迁移。Writer 只接受协议 parser 可识别的 module/provider archive，在数据库租约内校验 archive 结构并按 Nexus 公开 path 幂等写入，不创建 hosted Provider platform、revision 或 signing state。Remote service、metadata、route、validator、checksum manifest 和 signature snapshot 不从源端照搬；目标端下次 metadata 请求从已配置上游重建快照，Provider archive 在提供给客户端前按当前 `shasum` 复核。
...
| Nexus hosted data、显式 proxy cache 与 proxy/group 配置迁移 | 源数据/配置 | 必须 | 迁移前后 SHA-256 + 迁移后真实 `terraform init` |
...
7. M6：Admin、Browse、上传和迁移
   ...
   - 完成 Nexus definition/hosted full migration，以及显式选择的原生 Terraform proxy archive cache 迁移；用独立 proxy writer 保留 Nexus path，并从目标上游重建 route/checksum/signature snapshot。
...
- Nexus Terraform hosted 数据可 dry-run、resume、校验并迁移；proxy/group 配置可迁移；显式选择且计划为 `FULL` 的原生 Terraform proxy archive cache 使用独立写入分支迁移。迁移 E2E 必须真实预热 Nexus proxy、在目标 metadata 回源前校验同 path/SHA-256 的 asset 已存在，并通过真实 `terraform init` 验证重建后的 checksum/signature snapshot。
