...
5. 管理、搜索和迁移
   ...
   - Nexus metadata migration 识别 Terraform repository definition；repository data migration 对 hosted 支持 dry-run、resume、checksum、报告和 source profile gate。Proxy/group 配置可迁移；proxy cache data backup 在 route/validator/checksum/signature snapshot 证据完整前必须 fail fast。
...
- Proxy cache：当前 source profile 只能证明通用 datastore 表结构，不能证明 remote service、cached metadata、remote route、validator、checksum/signature 属于同一快照，因此只迁移配置；选择 Terraform proxy 做 repository-data backup 时必须在创建 job 前返回 `CONFIG_ONLY` 并 fail fast，不能把 archive 当 hosted 内容重放。
...
| Nexus hosted data 与 proxy/group 配置迁移 | 源数据/配置 | 必须 | 迁移后 `terraform init` |
...
7. M6：Admin、Browse、上传和迁移
   ...
   - 完成 Nexus definition/hosted full migration；在 source profile 能证明 route/validator/checksum/signature snapshot 前，显式拒绝 proxy cache data backup。
...
- Nexus Terraform hosted 数据可 dry-run、resume、校验并迁移；proxy/group 配置可迁移，Terraform proxy cache data backup 会在 job 创建前明确拒绝；迁移后真实 `terraform init` 成功。
