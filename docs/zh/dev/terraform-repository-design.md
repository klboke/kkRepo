...
- Token segment 必须在所有 access/error 日志中脱敏；脱敏识别必须先于仓库存在性和 format 判断，删除中的仓库、未知仓库或错误 format 也不能泄露 Terraform-shaped URL 中的 token。审计只记录 token id、principal、repository、operation 和结果，不记录 token material。
