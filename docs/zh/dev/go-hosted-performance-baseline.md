...
8 个预热读取场景分别先执行 32 次 warmup，再以并发 16 请求 250 次；共执行 3 轮并交替目标
顺序，表格取各轮统计量的中位数。发布场景每轮向每端并发 8 路发布 24 个唯一 module，覆盖
archive 校验、3 个 blob 写入和 `.mod`/`.info`/`.zip` 原子 metadata 事务。本次记录包含
12,144 个计时 HTTP 请求，全部正确性检查和计时请求均成功。
...
凭据只用于 HTTP header，不会写入 JSON 报告。
