...
5. 在数据库事务中创建 `WAITING` import task 和 marker/outbox，记录 coordinate、staging blob 与调用者；同一条 INSERT 通过 `(repository, active coordinate hash)` 唯一约束原子预留非终态 coordinate。预留失败按 Nexus 行为返回 `400` 并解除本请求 staging 引用，只有预留成功才返回对应的 `202` 和 `task` URL。
...
- 标准 multipart publish 先用 canonical filename 对 immutable coordinate 做轻量预检；通过后只流式写一次 staging blob，并在 durable task INSERT 中原子预留 active coordinate，只有胜出的请求返回 `202`。Worker 原子批量 claim task，并直接 promote 同一 blob 引用；promotion 阶段不复制临时文件、不重复上传或再次执行 hash pass。
