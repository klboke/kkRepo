...
5. 在数据库事务中创建 `WAITING` import task 和 marker/outbox，记录 coordinate、staging blob 与调用者；返回 Nexus 对应的 `202` 和 `task` URL。
...
9. task 标记 `COMPLETED`、写 `finished_at`；失败则保存稳定 error/message 并释放无引用 staging blob。
...
- 标准 multipart publish 先用 canonical filename 对 immutable coordinate 做轻量预检；通过后只流式写一次 staging blob，创建 durable task 后立即返回 `202`。Worker 原子批量 claim task，并直接 promote 同一 blob 引用；promotion 阶段不复制临时文件、不重复上传或再次执行 hash pass。
