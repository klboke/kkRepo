...
- `ansible_import_task`
  - 保存 task UUID、repository、发起主体、staging blob、expected/actual SHA-256、state、messages/error、attempt、lease owner/expiry、fencing token、created/started/finished 时间；非终态 task 还保存有唯一约束的 active coordinate hash，作为跨副本发布预留。
...
2. 从强制 canonical archive filename 解析 coordinate；已存在的 immutable version 立即按 Nexus 行为返回 `400`，不写 staging、不创建 task。
3. 流式写入 blob store 的 staging key，同时计算 SHA-256；失败请求清理未引用 staging blob。
4. 校验 multipart `sha256`、filename 和请求级限制。
5. Multipart 建任务、raw PUT 与 worker promotion 共用 fenced coordinate lease；在 lease 内同时复查 immutable version 和 active task。在数据库事务中创建 `WAITING` import task 和 marker/outbox，记录 coordinate、staging blob 与调用者；同一条 INSERT 通过 `(repository, active coordinate hash)` 唯一约束原子预留非终态 coordinate。预留失败按 Nexus 行为返回 `400` 并解除本请求 staging 引用，只有预留成功才返回对应的 `202` 和 `task` URL。
6. worker 通过数据库 lease/fencing claim task，执行 tar/manifest/files/metadata 校验。
7. 在事务中插入 collection version、component、asset、asset_blob、signature 和 browse node；coordinate 唯一约束裁决并发重复发布，version 上的 task provenance 只允许同一 task 在提交后崩溃时幂等恢复。
8. 原子切换 staging blob 为正式引用，提交 version revision，并失效 collection/version/group metadata cache。
9. task 标记 `COMPLETED`、写 `finished_at` 并在同一 fenced UPDATE 中释放 active coordinate 预留；失败也原子释放预留、保存稳定 error/message 并解除无引用 staging blob。成功路径此时已有 immutable version 唯一约束接管 coordinate。
...
- 同一 coordinate 并发 multipart publish 最多一个请求得到 `202`；multipart、raw PUT 和 worker version commit 之间通过同一 lease 完成交接，其余请求返回 conflict，不得产生两个 artifact blob 引用。
...
- 标准 multipart publish 先用 canonical filename 对 immutable version/active reservation 做轻量预检；通过后只流式写一次 staging blob，并在共享 coordinate lease 内复查后通过 durable task INSERT 原子预留，只有胜出的请求返回 `202`。Raw PUT 和 worker commit 进入同一 lease，关闭 reservation 到 immutable version 的交接窗口。Worker 原子批量 claim task，并直接 promote 同一 blob 引用；promotion 阶段不复制临时文件、不重复上传或再次执行 hash pass。
