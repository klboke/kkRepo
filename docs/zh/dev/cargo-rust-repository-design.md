...
- kkrepo 第一阶段应只实现 sparse registry。Cargo git index 需要维护可被客户端 clone/fetch 的 git 仓库，和 kkrepo 的共享数据库真相、OSS/S3 blob 模型不匹配，且 Nexus 参考行为不要求第一阶段支持。
- Cargo sparse index 是大量小 HTTP 请求。服务端支持 `ETag`、`Last-Modified` 和 `304 Not Modified`，并用生成的 body hash 和数据库持久化的 updated timestamp 作为稳定缓存标识。
...
- Cargo publish 成功后客户端会轮询 index 等待新版本出现。kkrepo hosted publish 应在返回成功前完成共享数据库版本行和 sparse index 可见状态的事务提交，避免客户端短时间内看不到刚发布的版本。
...
- proxy `config.json` 和 sparse index 文件以 metadata asset 方式缓存，body 和远端 validator（`ETag`、`Last-Modified`）通过共享数据库 asset/blob 引用和 OSS/S3 字节持久化。
...
- Published crate version、index JSON、yank 状态、proxy revalidate 状态和仓库配置保存在共享数据库 component/asset/repository 行中。
...
- Proxy revalidate 和远端 `.crate` 下载在并发下可能重复请求远端，但正确性由数据库 asset/component 唯一约束和 checksum 校验保护。
