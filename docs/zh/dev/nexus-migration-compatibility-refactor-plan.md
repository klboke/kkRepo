...
| `formatCapabilities` | 格式专项能力，例如 Maven metadata、npm token、Cargo sparse index、Pub package metadata/upload session、Docker connector |
| `warnings` | 展示给用户的风险提示 |
...
- Maven、npm、PyPI、Go、Helm、NuGet、RubyGems、Yum、Docker、Cargo 和 Pub 都必须有探测证据和校验逻辑后再启用。
