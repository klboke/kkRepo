...
kkrepo 将元数据和协调状态存储在 MySQL 或 PostgreSQL，将制品字节内容存储在 blob storage。可恢复的生产部署必须同时备份两者。
...
数据库保存引用和 checksum，blob storage 保存真实字节。只恢复其中一边不是完整恢复。

## 备份策略

推荐策略：

- 使用自动数据库备份，条件允许时启用 point-in-time recovery。
...
小规模部署可能每天一次数据库备份加 blob-storage versioning 就足够。承担核心 CI/CD 基础设施时，应缩短备份间隔，并做明确恢复演练。
...
- 时间点 `T` 的数据库 snapshot。
- 包含数据库在时间点 `T` 引用的所有对象的 blob storage 状态。

Blob storage 中存在额外未引用 object 通常可以接受，因为数据库控制可见仓库内容。数据库引用的 blob object 缺失不可接受，会导致下载失败。
...
使用 `pg_restore --clean --if-exists --dbname=<target> <dump>` 恢复到空目标库。生产环境优先使用托管 PostgreSQL snapshot/PITR。
...
不要把 lifecycle 删除配置得过于激进，以免数据库备份保留期内仍被引用的对象提前消失。
...
如果对象存储已经包含所需对象的超集，可能只需要恢复数据库并验证 blob 可用性。
...
1. 创建一个全新环境。
2. 恢复接近生产规模的 MySQL 或 PostgreSQL 备份。
