...
该 suite 会通过 Maven、npm、PyPI、Go、Helm、Cargo/Rust、Dart/Pub、Composer/PHP、Terraform 0.13/当前稳定版、SwiftPM/Xcode、Ansible Galaxy 2.9/当前版、Conda、APT/Debian、NuGet、RubyGems、Yum、Docker/OCI 客户端发布/上传并下载/解析。Composer 额外覆盖 hosted 到 proxy 的传递依赖、错误 Basic 凭据和清空客户端缓存后的断上游 lock replay；Terraform 覆盖 group 中的 hosted 与 registry.terraform.io proxy module/provider，并验证 Provider checksum/signature metadata；Swift 5.7 覆盖 registry/proxy resolve/build，5.10/6.x 额外覆盖 HTTPS login、不可变发布、hosted/group、SCM replacement 和跨副本读取；Ansible 覆盖 Galaxy v3 discovery、持久化 publish task、不可变版本、range/transitive dependency、hosted/group 下载、public Galaxy proxy、Basic/GenericToken/Nexus bearer 认证和跨副本读取；Conda 覆盖 hosted 上传、group search/create/list、上游 proxy search/create/list、repodata 变体和 cleanup；APT 覆盖三个客户端镜像、带认证签名安装/升级/删除、key rotation、passthrough/re-sign proxy 和跨副本读取。Swift 独立 lane 覆盖 macOS Xcode 与 Windows proxy resolve；Go 向 hosted 发布规范内部 module ZIP，通过 group 解析，并使用同一个 group 验证公共 module 的 proxy fallback。客户端诊断信息会写入 `artifacts/client-e2e/`，包含 credential 的 metadata 会在上传 artifact 前脱敏。
...
- [Go 仓库使用指南](repository-guides/go.md)与 [Nexus 性能基线](dev/go-hosted-performance-baseline.md)
