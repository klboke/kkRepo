...
Dart / Pub 之外，部分 component upload specs 和 security/admin contracts 仍会对比 Nexus 旧 UI/internal API 入口。kkrepo 的生产和发行配置默认保持 `kkrepo.nexus.legacy-ui.enabled=false`，以关闭 Nexus Rapture、ExtDirect、Wonderland、internal UI upload 等旧 UI 兼容面；但一次性的 compatibility compose 环境会把候选服务的 `KKREPO_NEXUS_LEGACY_UI_ENABLED` 默认设为 `true`，专门用于这些 Nexus 旧入口兼容测试。手工复现这类 live 测试时，必须在启动候选 kkrepo 服务前设置该环境变量；只在 `mvn -pl compat-test ... test` 前设置不会改变已经启动的服务。正常部署仍应保持默认关闭，kkrepo 自身 Browse UI 使用 native `/internal/security/permissions`，不依赖 legacy internal UI 权限接口。
