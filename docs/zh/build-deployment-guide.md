...
`KKREPO_RUNTIME=jvm` 选择 `ghcr.io/klboke/kkrepo:1.0.2`，`KKREPO_RUNTIME=native` 选择 `ghcr.io/klboke/kkrepo:1.0.2-native`。两种镜像均发布 Linux `amd64` 和 `arm64` 架构。显式设置 `KKREPO_IMAGE_TAG` 时，会覆盖对应运行时的默认 tag。
...
- `ghcr.io/klboke/kkrepo:1.0.2` 中的 JVM 运行时
- MySQL 8.0
...
```bash
docker pull ghcr.io/klboke/kkrepo:1.0.2
```
...
```bash
docker pull ghcr.io/klboke/kkrepo:1.0.2-native
