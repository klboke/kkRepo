...
- Conda 迁移后，校验 hosted/proxy/group definition 与 member 顺序、hosted package 原始字节/SHA-256，以及目标端生成的 JSON/BZ2/ZSTD repodata。通过迁移后的 group 和代表性 proxy channel 执行 `conda search/create/list`；未知 source profile/shape 或无法证明 package identity 的内容必须保留为 manual action，不能猜测导入。
- 管理员登录。
- Browse/search。
- 用户角色和仓库权限。
- CI token/API key 行为。
