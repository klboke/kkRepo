...
每个 `(namespace, name, version)` 都不可变。Multipart 路径从强制 canonical archive filename 得到 coordinate，并在写 staging 前拒绝已发布版本；创建 task 时会在数据库中原子预留 active coordinate，因此并发中的重复请求不会再得到第二个 `202`，失败请求产生的 staging 引用会立即解除。Task 保留发布来源标识，使崩溃恢复可幂等完成。即使仓库 write policy 允许更新，重复发布同一 version 也会失败；请在 `galaxy.yml` 中提升版本。
